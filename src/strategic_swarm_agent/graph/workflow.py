...
try:
    ...
    Exception
):  # pragma: no cover - fallback for incompatible local langgraph installs
    END = "__end__"
    START = "__start__"
    ...
    class StateGraph:  # type: ignore[override]
        ...
        def add_conditional_edges(
            self, source: str, fn, mapping: dict[str, str]
        ) -> None:
...
class StrategicSwarmWorkflow:
    ...
    def build(self, *, _input_schema=None):
        graph = (
            StateGraph(SwarmGraphState, input_schema=_input_schema)
            if _input_schema
            else StateGraph(SwarmGraphState)
        )
    ...
    def ingest_signals(self, state: SwarmGraphState) -> SwarmGraphState:
        ...
        if state.get("run_mode") == "demo":
            ...
            raw = (
                self.news.fetch(scenario_id)
                + self.market.fetch(scenario_id)
                + self.dark.fetch(scenario_id)
            )
            provenance = [
                f"Ingested {len(raw)} raw signals from sample providers for {scenario_id}."
            ]
            return {
                "raw_signals": raw,
                "provenance": provenance,
        ...
        end_at = (
            datetime.fromisoformat(state["window_end"])
            if state.get("window_end")
            else now
        )
        raw = []
    ...
    def normalize_events(self, state: SwarmGraphState) -> SwarmGraphState:
        ...
        prior_story_counts = (
            self.store.get_story_counts(story_keys)
            if state.get("run_mode") != "demo"
            else {}
        )
    ...
    def pattern_match(self, state: SwarmGraphState) -> SwarmGraphState:
        ...
        cluster_events = (
            [
                item
                for item in state["normalized_events"]
    ...
    def enrich_dark_signals(self, state: SwarmGraphState) -> SwarmGraphState:
        ...
        if self.web_search.enabled:
            all_clusters = state.get("event_clusters") or (
                [] if not cluster else [cluster]
            )
            eligible = sorted(
                [
                    c
                    for c in all_clusters
                    if len(c.signal_ids) >= self.web_search.min_cluster_signals
                ],
            ...
            for c in eligible:
                question = self.web_search.build_query(
                    ...
                    consequence_hint=scenario.consequence_chain[:2]
                    if scenario
                    else None,
                )
                try:
                    web_search_signals.extend(
                        self.web_search.query(
                            question, story_key=c.story_key, fetched_at=fetched_at
                        )
                    )
        ...
        if self.web_search.enabled and opportunities:
            ...
            for opp in opportunities[:3]:  # cap at 3 to avoid excess API calls
                ...
                try:
                    ...
                    if signals:
                        opp.search_summary = (
                            signals[0].summary[:400] if signals[0].summary else None
                        )
                except Exception:  # noqa: BLE001
                    pass
    ...
    def compute_fragility(self, state: SwarmGraphState) -> SwarmGraphState:
        ...
        diagnostics = {
            **state.get("diagnostics", {}),
            "fragility_score": assessment[0].fragility_score.value
            if assessment
            else 0.0,
        }
        provenance = (
    ...
    def generate_opportunities(self, state: SwarmGraphState) -> SwarmGraphState:
        ...
        ideas = self.opportunity_generator.generate(
            state["ripple_scenarios"], retry_count=retry_count
        )
        provenance = [
            *state["provenance"],
    ...
    def critique_execution(self, state: SwarmGraphState) -> SwarmGraphState:
        reviewed = self.execution_critic.review(
            state["candidate_opportunities"], state["selected_cluster"]
        )
    ...
    def _route_after_critique(
        self, state: SwarmGraphState
    ) -> Literal["retry", "synthesize"]:
        approved = sum(1 for item in state["reviewed_opportunities"] if item.approved)
        if approved == 0 and state.get("opportunity_retry_count", 0) <= state.get(
            "max_opportunity_retries", 1
        ):
            return "retry"
        return "synthesize"
