...
class SignalAlchemist:
    ...
    def enrich(
        self, events: list[SignalEvent], cluster: EventCluster
    ) -> tuple[list[SignalBundle], dict]:
        if not events:
        ...
        sentiment_entropy_value = min(
            1.0, 0.2 + distinct_dark_tags / 8 + dark_ratio / 3
        )
        ...
        if len(dark_events) < 2:
            uncertainty_notes.append(
                "Dark-signal coverage is shallow, so sentiment entropy is probably understated."
            )
