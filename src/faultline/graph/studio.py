...
Exposes the Faultline graph for use in LangGraph Studio / LangGraph Cloud.

Usage in langgraph.json:
    "graphs": { "faultline": "faultline.graph.studio:graph" }
...
from faultline.models import FaultlineInputSchema  # noqa: E402
from faultline.persistence.store import SignalStore  # noqa: E402

_output_dir = Path(os.getenv("FAULTLINE_OUTPUT_DIR", "outputs"))
...
# _input_schema=FaultlineInputSchema restricts the Studio Input panel to the 4 user-facing fields
# (scenario_id, run_mode, window_start, window_end) without affecting programmatic invocations.
graph = _workflow.build(_input_schema=FaultlineInputSchema)
