...
from faultline.utils.config import load_archetypes, load_scoring_config
...
class FragilityScorer:
    ...
    def score(
        ...
        redundancy_value = self._bounded(
            0.18 + 0.12 * len(tags.intersection({"bypass", "microgrid", "open-source", "redundancy", "protocol"}))
        )
